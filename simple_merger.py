...
def get_kv_and_q_indices(*retrieval_head_args, **retrieval_head_kwargs):
    ...
    keep_ratio = len(retrieval_heads) / 1024  ## hard_coded
    print("Keep ratio: ", keep_ratio)

    for retrieval_head, _ in retrieval_heads:
        layer, head = retrieval_head
...
def do_model_merge():
    ...
    retrieval_heads, head_indices_to_mask_kv, head_indices_to_mask_q = get_kv_and_q_indices(cutoff=cutoff)

    retrieval_head_distribution = [0] * num_layers
    ...
    print("Retrieval head distribution: ", retrieval_head_distribution)

    #####################

    if merge_model1:
        if merging_method in ["double_sample", "double_sample_exclusive"]:
            if merging_method == "double_sample":
        ...
        for param_name, param_value in base_model_param_dict.items():
            ...
            if 'self_attn' in param_name and "_proj" in param_name:  # Dealing with attention heads
                ...
                if merging_method == "simple_substitution":
                    ...
                    use_delta = True
                    use_delta1 = False
                elif merging_method == "double_delta":
                ...
                if use_delta:
                    ...
                    masked_delta = mask_input_with_mask(delta, mask, use_rescale=rescale, mask_rate=None)
                    if merge_model1:
                        # drop and scale
                        if use_delta1:
                ...
                if substitute_flag:
                    ...
                    if merge_model1:
                        ...
                        merged_params[param_name] = (base_model_param_dict[param_name], None)
